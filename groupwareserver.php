 ...
 */
$baseUri = '/~evert/sabre/caldavtester/groupwareserver.php/';
 ...
 */
/*
$authBackend      = new \Sabre\DAV\Auth\Backend\BasicCallBack(function($userName, $password) use ($pdo) {
    // Basic auth that uses the PDO database table for digest.
    $query = "SELECT id FROM users WHERE username = ? AND digesta1 = ?";
    $stmt = $pdo->prepare($query);
    $stmt->execute([$userName, md5("$userName:SabreDAV:$password")]);
